...
/**
 * struct sail_list_socket_t the socket type options
 * along with the normal recognizable name
 */
const struct sail_list_socket_t {
	//socket type
	int sock_type;
 ...
 */
static const char *get_socket_type_descr(const int opt_type)
{
	const char *unknown_dscrp = "Uknown SOCK_XXX socket type";
	const struct sail_list_socket_t *ptr;

	for (ptr = types; ptr->dscrp != NULL; ptr++) {
...
/**
 * char* sail_ntop 
 * function supports only AF_INET,AF_INET6
 * and AF_UNIX aka AF_LOCAL
 */
char *sail_ntop(int sockfd, const struct sockaddr *sa, socklen_t len)
{
	...
	case AF_INET: {
		...
		sail_getsockopt(sockfd, SOL_SOCKET, SO_TYPE, &rtype, &len);
 ...
 */
void tell_info_hosts(int n, char **host_names)					  // only for ipv-4
{
	char *ptr, **pptr;
	char str[BUFFSIZE];
	hostent *hptr;
...
void Getaddrinfo(const char *hostname, const char *service,
